...
[MemoryDiagnoser]
public class ChunkFileWriterBenchmark
{
	// file will be appended with constant line, length - 90 chars
	
	[Params(1 << 16, 1 << 18, 1 << 20, 1 << 22)] public int BufferSize { get; set; }
	//[Params(1 << 18)] public int BufferSize { get; set; }
	[Params(1 << (20 + 8 - 7))] public int LinesCount { get; set; }

	//[Benchmark]
	public void WriteWithStringBuilder()
	{
	...
	//[Benchmark]
	public void WriteWithNewStringBuilder()
	{
		using var writer = new StreamWriter(
			$"test_{BufferSize}.txt", 
			Encoding.UTF8, 
	...
	public void WriteWithString()
	{
		...
		// check if needed
		writer.Flush();
		Console.WriteLine($"file created with length {writer.BaseStream.Length / 1024 } KB");
	}
}